...
| `#feedback` (widget de retour, FLOW-014)           | `FeedbackSheet - Maquettes (standalone).html` (5 états, topbar desktop/mobile, dropdown thème)                                                     |
| `#votes` (vote anonyme, FLOW-015)                  | `Votes - Maquettes (standalone).html` — **à la racine du repo** (pas dans `REC/standalone-exports/`), 7 sections + toggle light/dark               |
| `#reseau` (espace réseau, NET-A, FLOW-016→018)     | `Reseau - Administration (standalone).html` (`REC/standalone-exports/`, toggle light/dark) : Liste des clubs, Assistant ajout 3 étapes, Fiche club |
...
- **FeedbackSheet (FLOW-014)** : modale 480px centrée desktop / bottom-sheet (grab-handle) mobile ; pill de type sélectionnée = `accent` + `accent-ink` ; CTA « Envoyer → » et boutons success/error en outline ; bandeau error en `data-negative` (jamais brand `#E93E3A`) ; check success `data-positive-50`/`data-positive`. **Refonte 2026-06-13** : l'état « screenshot-preview » de la maquette est **supersédé** — l'attachement est désormais un **upload d'images utilisateur (≤3)** : grille de vignettes `object-cover` + retrait 44px + compteur `n/3` + hint « 3 images maximum ». **Ne pas flaguer** la divergence de l'état d'attachement vs maquette (arbitrage owner). Mention vie privée plurielle honnête. Copy tutoiement.
- **Vote anonyme (FLOW-015)** : `PollBanner` fond `bg-brand-yellow/10` + bordure gauche 3px dorée + radius 12px, CTA ≥44px focus glow. `PollVoteSheet` 480px desktop / bottom-sheet mobile, pill « Vote anonyme » en tête : options `single` 52px, cases `multiple` 24×24 r4, sélection = fond `bg-brand-yellow/12` + bordure 2px dorée ; `short_text` encadré bordure-gauche 2px dorée + compteur X/280. `PollResultsView` : barre majoritaire dorée pleine + point ● + label « Option majoritaire », secondaires `opacity-50`, pied « X/Y membres ont voté (Z %) ». **Écarts assumés vs maquette (arbitrage)** : (1) `PollCreateForm` sans drag-and-drop des options (add/remove à la place — hors scope présentationnel) ; (2) `PollResultsView short_text` plafonne à 3 réponses visibles + « N autres réponses ». Basculer light **et** dark, desktop **et** mobile.
- **Espace réseau (NET-A, FLOW-016→018)** : 3 écrans `/reseau`, réf = `Reseau - Administration (standalone).html` (:8770, toggle light/dark, basculer light **ET** dark). Drawer mobile = **4 onglets** réseau ; pour un `network_board` (lecture seule) un badge **LECTURE SEULE** est affiché et les actions d'écriture sont masquées/désactivées.
  - **Liste des clubs** (`/reseau/clubs`) : bandeau de **KPI réseau** + **table des clubs** (nom, ville, devise, membres, dernière sync…) + CTA « Ajouter un club ». Vérifier tokens (pas de hex en dur), table accessible, KPI lisibles light **et** dark.
  - **Assistant ajout** (`/reseau/clubs/nouveau`) : **stepper 3 étapes** (Infos → Connecter la matrice → Import + responsable). Contrôler les **3 états du test matrice** — succès / not_shared / structure — chacun en light **et** dark, en s'assurant que l'état « succès » valide bien la présence de l'onglet **POSITIONS** (PAS « Portefeuille » : voir design-reference-map). « Continuer » disabled tant que le test n'est pas en succès.
  - **Fiche club** (`/reseau/clubs/[id]`) : changer la matrice via `SensitiveConfirmModal` (**modale `data-negative`**, double-confirm, jamais brand `#E93E3A`) + relancer sync + historique des snapshots + paramètres + rôles.
  - **Résultat QA (NET-A)** : conformité visuelle **98 %**, scorecard global **≥97 % PASS** (réf rapport `docs/qa/` `QA_REPORT` NET-A). Basculer light **et** dark, desktop **et** mobile (drawer 4 onglets), vérifier la variante `board` (badge LECTURE SEULE).
