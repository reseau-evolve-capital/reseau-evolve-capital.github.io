...
- **BUG-VA-5 (corrigé — RPC + data + web)** : participation affichait `voted/voted = 100 %`. Un membre ne peut PAS compter ses pairs via RLS (privacy `memberships`). Fix : `get_poll_results` (SECURITY DEFINER) renvoie désormais `eligible_members` (membres actifs du club), mappé en `PollResults.eligibleMembers`, consommé par `PollDetailView`. Vérifié runtime : **« 3/4 membres ont voté (75,00 %) »**.

**Vérifié OK au runtime (membre + admin, login réel)** : `/votes` (4 votes, badges, types) ; `PollVoteSheet` yes_no (badge « Vote anonyme », radios Oui/Non/Abstention, « Confirmer » désactivé sans sélection) ; **soumission bout-en-bout** (Server Action → RPC `submit_vote` → INSERT confirmé en DB) ; `PollResultsView` (libellé localisé « Oui », ● majoritaire 67/33 %, **participation 3/4 = 75 %**) ; **bannière dashboard** (variante agrégée « 3 votes en attente → Voir tous », au-dessus des KPI) ; **entrée « Votes » topbar** (badge count) ; **`/admin/votes`** (onglets En cours/Brouillon/Clôturé, `AdminPollRow` avec participation `3/4 · 75 %` / `—` after_close / `0/4`, « Voir résultats » + « Clôturer », CTA « Nouveau vote ») ; **`/admin/votes/nouveau`** (`PollCreateForm` step 1 : titre, description, 4 radios de type, « Continuer » désactivé tant qu'invalide). **Gate final `make lint typecheck test` → exit 0**.

### Finding environnement (hors périmètre vote, à traiter)

- **Grants Data API manquants sur les tables pré-037 en LOCAL** : ce DB local (démarré après le 30/05/2026) n'a **aucun** `GRANT SELECT` `authenticated` sur `users`/`clubs`/`contributions`/`memberships`/… (auto-expose off). Conséquence : en local, le middleware ne lit pas `users.onboarding_completed` → redirige `/dashboard` vers `/onboarding` ; le dashboard ne charge pas. **Toute l'app membre est dégradée en local**, pas seulement les votes. En prod les grants existent (tables créées avant la bascule). Grants posés en local (DB de dev) pour vérifier ; **ticket : migration de grants explicites pour toutes les tables Data API** (sinon tout nouveau DB local post-bascule est cassé).

### Reste à faire (non terminé)
