...
- **Mécanique A/B** : `getDashboardVariant()` server-only (`apps/web/lib/experiments/dashboard-v2.ts`), seul point de branchement = `page.tsx`. Précédence `DASHBOARD_V2_FORCE` (env) > cookie `ec_dashboard_variant` (lecture seule, QA/e2e — **jamais écrit par l'app** : `cookies().set()` interdit en RSC Next 16 et middleware gelé ; le hash FNV-1a par `userId` est plus sticky qu'un cookie) > hash vs `DASHBOARD_V2_ROLLOUT` (défaut **0** = fail-safe V1 ; activation prod = poser 50 sur Vercel). **Pas de SDK `flags`** (arbitrage : pas d'Edge Config configuré, `FLAGS_SECRET` = friction e2e, un `decide()` custom aurait été nécessaire de toute façon) — signature « decide-like » pour migration future.
...
- **Données DEMO (décision V0 débloquée côté UI uniquement)** : la courbe vient de `dashboard-chart-demo.ts` (séries déterministes seedées, dernier point = quote-part réelle, cibles design 30J +4,55 % / MAX +178 % depuis 2018) avec micro-label **« Courbe illustrative »** permanent ; `chart_data_source: 'demo'` dans GA4. Le pipeline réel (`member_quote_part_snapshots` + extension `getDashboardData`) reste un **ticket follow-up**. **V1 strictement inchangée** (aucun mock, aucun graphe — diff limité au hook analytics `dashboard_viewed`).
