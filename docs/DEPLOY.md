...
## 1. État post-migration (✅ fait — PR #16)

> **Migration terminée.** `main` **est** le monorepo. La branche `feat/monorepo` est historique.
> Cette section documente l'impact du merge et reste utile pour le déploiement vitrine.

Avant merge : `main` = vitrine legacy à la racine ; `feat/monorepo` = monorepo en branche longue.
Après merge : `main` = monorepo (`apps/vitrine` + `apps/web` + packages + Supabase).
...
3. Merger la PR vers `main` (déjà fait). `ci.yml` / `lighthouse.yml` tournent sur push et PR vers `main`.
...
### Annexe — workflows (`main`)

- `ci.yml` : lint + typecheck + test + build `apps/web` sur **push et PR** → `main`. Gate qualité. ⚠ **Ne couvre PAS les tests Deno ni e2e** (les lancer à part, cf. `docs/qa/README.md`). Ne couvre **pas** `apps/cms` (hors workspace pnpm).
