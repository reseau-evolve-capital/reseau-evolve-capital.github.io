...
// Erreur de chargement de /reseau/newsletter (ex. lecture CMS / rendu en échec). État error
// explicite (jamais d'écran vide / crash) + bouton réessayer. Cf. CLAUDE.md (états error/empty).
export default function ReseauNewsletterError({
  error,
  reset,
}: {
  error: Error
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-col items-center gap-4 px-4 py-16 text-center">
      <RouteErrorReporter error={error} routeSegment="reseau/newsletter" />
      <h2 className="font-display text-[18px] font-bold text-text">
        {t('reseau.newsletter.error.title')}
      </h2>
