// GET /api/contributions — cotisations du membre courant (COT-004).
//
// Garde-fous :
//   - authentification via session cookie Supabase → 401
//   - résolution du club : ?club_id=… sinon dernière adhésion active → 404 si aucun
//   - RLS isole les données par auth.uid() (policies « contributions: own read » + « cm: own read »)
//   - aucune ligne contributions → 404 (état empty côté UI)
//
// JAMAIS de service-role ici — la RLS doit s'appliquer (données nominatives).
// Réf : COT-004, ARCHITECTURE.md §1, DATA_MODEL.md §2.6/§2.7, CLAUDE.md (RLS).
