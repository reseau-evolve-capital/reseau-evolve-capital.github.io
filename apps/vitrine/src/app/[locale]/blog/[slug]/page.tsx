...
export default async function ArticlePage({ params }: ArticlePageProps) {
  ...
  // Nouveau rendu éditorial si `corps` (dynamic zone) est peuplé ; sinon
  // rendu legacy `content` (blocks) — on ne casse pas les articles existants.
  const hasEditorialBody = Boolean(article.corps && article.corps.length > 0)
  ...
  return (
    <article className="container mx-auto px-4 py-16">
      {hasEditorialBody ? (
        ...
        <BlogArticleContent article={article} locale={locale} translations={translations} />
      )}

      {/* Related Articles */}
      <div className="mx-auto max-w-6xl">
        <RelatedArticles currentArticle={article} locale={locale} maxArticles={3} />
      </div>
    </article>
  )
}
